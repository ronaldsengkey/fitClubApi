    ...
    return new Promise(async function (resolve, reject) {
        try{
            ...
            db.query(query,[param.email],async function(err,res){
                try{
                    if(err){
                        message = {"responseCode":process.env.ERRORINTERNAL_RESPONSE,"responseMessage":"Error login ", err}
                        resolve(message);
                    }else{
                        if(res.length > 0){
                            message = {"responseCode":process.env.SUCCESS_RESPONSE,"data":res[0]};
                            resolve(message);
                        }else{
                            message = {
...
async function updateToken(param){
    try{
        ...
        db.query(query,[param.token, param.id, param.email, param.phone]
